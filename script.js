...
async function updateSequence(midiBuffer, fragmentIndex, variantIndex, player, visualizer) {
  const sequence = await sequenceFromFragmentIndex(midiBuffer, fragmentIndex, variantIndex);
  player.noteSequence = sequence;
  visualizer.noteSequence = sequence;
}

async function sequenceFromFragmentIndex(midiFileBuffer, fragmentIndex, phraseIndex) {
...
updateSequence(midiBuffer, 0, 0, player, visualizer);
...
loadbutton.addEventListener("click", function () {
  ...
  updateSequence(midiBuffer, fragmentInput.value, phraseInput.value, player, visualizer);
})
